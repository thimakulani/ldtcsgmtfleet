...
function departments_insert(&$error_message = '') {
	global $Translation;
	...
	if(function_exists('departments_before_insert')) {
		$args = [];
		if(!departments_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('departments', backtick_keys_once($data), $error);
	if($error) {
		$error_message = $error;
		return false;
	}
	...
	update_calc_fields('departments', $recID, calculated_fields()['departments']);

	// hook: departments_after_insert
	if(function_exists('departments_after_insert')) {
		$res = sql("SELECT * FROM `departments` WHERE `department_id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
	...
	if(strlen(Request::val('SelectedID'))) departments_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function departments_copy_children($destination_id, $source_id) {
	global $Translation;
...
function departments_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('departments', $selected_id, 'delete')) {
	...
	$res = sql("SELECT `department_id` FROM `departments` WHERE `department_id`='{$selected_id}'", $eo);
	$department_id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `gmt_fleet_register` WHERE `department_name`='" . makeSafe($department_id[0]) . "'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'gmt_fleet_register', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'departments_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'departments_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `department_id` FROM `departments` WHERE `department_id`='{$selected_id}'", $eo);
	$department_id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `gmt_fleet_register` WHERE `department_name_of_driver`='" . makeSafe($department_id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `department_id` FROM `departments` WHERE `department_id`='{$selected_id}'", $eo);
	$department_id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `claim` WHERE `department_name`='" . makeSafe($department_id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `department_id` FROM `departments` WHERE `department_id`='{$selected_id}'", $eo);
	$department_id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `vehicle_annual_inspection` WHERE `department_name`='" . makeSafe($department_id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	sql("DELETE FROM `departments` WHERE `department_id`='{$selected_id}'", $eo);
...
function departments_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('departments', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('departments', $selected_id);
	...
	if(function_exists('departments_before_update')) {
		...
		if(!departments_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('departments', $data['selectedID'], calculated_fields()['departments']);

	// hook: departments_after_update
	if(function_exists('departments_after_update')) {
		$res = sql("SELECT * FROM `departments` WHERE `department_id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['department_id'];
		$args = ['old_data' => $old_data];
...
function departments_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		$res = sql("SELECT * FROM `departments` WHERE `department_id`='" . makeSafe($selected_id) . "'", $eo);
	...
	if(($selected_id && !$AllowUpdate) || (!$selected_id && !$AllowInsert)) {
		$jsReadOnly = '';
		$jsReadOnly .= "\tjQuery('#department_name').replaceWith('<div class=\"form-control-static\" id=\"department_name\">' + (jQuery('#department_name').val() || '') + '</div>');\n";
	...
	// process images
	$templateCode = str_replace('<%%UPLOADFILE(department_id)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(department_name)%%>', '', $templateCode);

	// process values
	if($selected_id) {
		if( $dvprint) $templateCode = str_replace('<%%VALUE(department_id)%%>', safe_html($urow['department_id']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(department_id)%%>', html_attr($row['department_id']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(department_id)%%>', urlencode($urow['department_id']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(department_id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(department_id)%%>', urlencode(''), $templateCode);
	...
	$templateCode .= loadView('departments-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);
