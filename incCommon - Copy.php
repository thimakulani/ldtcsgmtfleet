	...
	*/
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			...
			'authorizations' => "`authorizations` LEFT JOIN `claim` as claim1 ON `claim1`.`claim_id`=`authorizations`.`job_code` LEFT JOIN `claim_status` as claim_status1 ON `claim_status1`.`claim_status_id`=`authorizations`.`job_status` LEFT JOIN `claim_category` as claim_category1 ON `claim_category1`.`claim_category_id`=`authorizations`.`job_category` LEFT JOIN `log_sheet` as log_sheet1 ON `log_sheet1`.`fuel_log_sheet_id`=`authorizations`.`job_odometer` LEFT JOIN `claim` as claim2 ON `claim2`.`claim_id`=`authorizations`.`pre_authorisation_date` LEFT JOIN `claim` as claim3 ON `claim3`.`claim_id`=`authorizations`.`vehicle_registration_number` LEFT JOIN `gmt_fleet_register` as gmt_fleet_register1 ON `gmt_fleet_register1`.`fleet_asset_id`=`claim3`.`vehicle_registration_number` LEFT JOIN `gmt_fleet_register` as gmt_fleet_register2 ON `gmt_fleet_register2`.`fleet_asset_id`=`authorizations`.`make_of_vehicle` LEFT JOIN `claim` as claim4 ON `claim4`.`claim_id`=`authorizations`.`client` LEFT JOIN `departments` as departments1 ON `departments1`.`department_id`=`claim4`.`department_name` LEFT JOIN `province` as province1 ON `province1`.`province_id`=`authorizations`.`province_name` LEFT JOIN `merchant` as merchant1 ON `merchant1`.`merchant_id`=`authorizations`.`merchant_code` LEFT JOIN `claim` as claim5 ON `claim5`.`claim_id`=`authorizations`.`total_claim` LEFT JOIN `claim` as claim6 ON `claim6`.`claim_id`=`authorizations`.`total_authorised` ",
			'service' => "`service` LEFT JOIN `service_item_type` as service_item_type1 ON `service_item_type1`.`service_item_type_id`=`service`.`service_item_type` LEFT JOIN `service_categories` as service_categories1 ON `service_categories1`.`service_categories_id`=`service`.`service_category` LEFT JOIN `merchant` as merchant1 ON `merchant1`.`merchant_id`=`service`.`merchant_name` LEFT JOIN `merchant_type` as merchant_type1 ON `merchant_type1`.`merchant_type_id`=`merchant1`.`merchant_type` LEFT JOIN `gmt_fleet_register` as gmt_fleet_register1 ON `gmt_fleet_register1`.`fleet_asset_id`=`service`.`vehicle_registration_number` LEFT JOIN `dealer` as dealer1 ON `dealer1`.`dealer_id`=`service`.`dealer_name` LEFT JOIN `log_sheet` as log_sheet1 ON `log_sheet1`.`fuel_log_sheet_id`=`service`.`closing_km` LEFT JOIN `work_allocation` as work_allocation1 ON `work_allocation1`.`work_allocation_id`=`service`.`work_allocation_reference_number` LEFT JOIN `schedule` as schedule1 ON `schedule1`.`schedule_id`=`service`.`date_of_service` LEFT JOIN `reception` as reception1 ON `reception1`.`reception_user_id`=`service`.`receptionist` LEFT JOIN `reception` as reception2 ON `reception2`.`reception_user_id`=`service`.`workshop_name` LEFT JOIN `districts` as districts1 ON `districts1`.`district_id`=`reception2`.`workshop_name` LEFT JOIN `year_model` as year_model1 ON `year_model1`.`year_model_id`=`gmt_fleet_register1`.`year_model_specification` LEFT JOIN `body_type` as body_type1 ON `body_type1`.`body_type_id`=`gmt_fleet_register1`.`type_of_vehicle` LEFT JOIN `application_status` as application_status1 ON `application_status1`.`application_id`=`gmt_fleet_register1`.`application_status` LEFT JOIN `departments` as departments1 ON `departments1`.`department_id`=`gmt_fleet_register1`.`department_name` ",
			...
			'driver' => "`driver` ",
			'accidents' => "`accidents` LEFT JOIN `gmt_fleet_register` as gmt_fleet_register1 ON `gmt_fleet_register1`.`fleet_asset_id`=`accidents`.`vehicle_registration_number` LEFT JOIN `log_sheet` as log_sheet1 ON `log_sheet1`.`fuel_log_sheet_id`=`accidents`.`closing_km` LEFT JOIN `driver` as driver1 ON `driver1`.`driver_id`=`accidents`.`drivers_surname` LEFT JOIN `dealer` as dealer1 ON `dealer1`.`dealer_id`=`accidents`.`dealer_name` LEFT JOIN `districts` as districts1 ON `districts1`.`district_id`=`accidents`.`district` ",
			...
			'vehicle_daily_check_list' => "`vehicle_daily_check_list` LEFT JOIN `gmt_fleet_register` as gmt_fleet_register1 ON `gmt_fleet_register1`.`fleet_asset_id`=`vehicle_daily_check_list`.`vehicle_registration_number` LEFT JOIN `log_sheet` as log_sheet1 ON `log_sheet1`.`fuel_log_sheet_id`=`vehicle_daily_check_list`.`closing_km` LEFT JOIN `driver` as driver1 ON `driver1`.`driver_id`=`vehicle_daily_check_list`.`drivers_surname` ",
			'auditor' => "`auditor` ",
			...
			'billing' => "`billing` LEFT JOIN `gmt_fleet_register` as gmt_fleet_register1 ON `gmt_fleet_register1`.`fleet_asset_id`=`billing`.`vehicle_registration_number` LEFT JOIN `dealer` as dealer1 ON `dealer1`.`dealer_id`=`gmt_fleet_register1`.`make_of_vehicle` ",
			'general_control_measures' => "`general_control_measures` LEFT JOIN `districts` as districts1 ON `districts1`.`district_id`=`general_control_measures`.`government_garage_station` ",
