<?php
	...
	$current_table = 'vehicle_daily_check_list';
			...
			<table data-tablename="<?php echo $current_table; ?>" class="table table-striped table-hover table-condensed table-bordered">
				...
				<tbody>
					...
					<tr data-id="<?php echo html_attr($pkValue); ?>">
						...
						<td class="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][31]}"; ?>" id="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][31]}-" . html_attr($record[$config['child-primary-key-index']]); ?>"><?php if($record[31]) { ?><a href="link.php?t=<?php echo $parameters['ChildTable']; ?>&f=document_checklist_report&i=<?php echo urlencode($record[$config['child-primary-key-index']]); ?>" target="_blank" class="btn btn-default" title="<?php echo html_attr($record[31]); ?>"><i class="glyphicon glyphicon-lg glyphicon-file text-info"></i></a><?php } ?></td>
