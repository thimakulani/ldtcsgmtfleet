...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$table_perms = getTablePermissions('schedule');
	...
	switch($mfk) {
		...
		case 'user_name_and_surname':
			if(!$id) {
				?>
				$j('#user_contact_email<?php echo $rnd1; ?>').html('&nbsp;');
			...
			?>
			$j('#user_contact_email<?php echo $rnd1; ?>').html('<?php echo addslashes(str_replace(["\r", "\n"], '', safe_html($row['user_contact_email']))); ?>&nbsp;');
		...
		case 'service_item_type':
			if(!$id) {
				?>
				$j('#service_item_type_code<?php echo $rnd1; ?>').html('&nbsp;');
				<?php
				break;
			}
			$res = sql("SELECT `service_item_type`.`service_item_type_id` as 'service_item_type_id', `service_item_type`.`service_item_type` as 'service_item_type', `service_item_type`.`service_item_type_code` as 'service_item_type_code' FROM `service_item_type`  WHERE `service_item_type`.`service_item_type_id`='{$id}' limit 1", $eo);
			$row = db_fetch_assoc($res);
			?>
			$j('#service_item_type_code<?php echo $rnd1; ?>').html('<?php echo addslashes(str_replace(["\r", "\n"], '', safe_html($row['service_item_type_code']))); ?>&nbsp;');
			<?php
			break;
