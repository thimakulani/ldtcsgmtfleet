...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$table_perms = getTablePermissions('purchase_orders');
	...
	switch($mfk) {
		...
		case 'part_number_1':
			if(!$id) {
				?>
				$j('#part_name_3<?php echo $rnd1; ?>').html('&nbsp;');
				$j('#part_name_4<?php echo $rnd1; ?>').html('&nbsp;');
				$j('#part_name_5<?php echo $rnd1; ?>').html('&nbsp;');
				<?php
				break;
			}
			$res = sql("SELECT `parts`.`parts_id` as 'parts_id', IF(    CHAR_LENGTH(`parts_type1`.`part_type`), CONCAT_WS('',   `parts_type1`.`part_type`), '') as 'part_type', `parts`.`part_number` as 'part_number', `parts`.`part_name` as 'part_name', `parts`.`description` as 'description', IF(    CHAR_LENGTH(`manufacturer1`.`manufacturer_name`), CONCAT_WS('',   `manufacturer1`.`manufacturer_name`), '') as 'manufacturer', IF(    CHAR_LENGTH(`dealer1`.`dealer_name`), CONCAT_WS('',   `dealer1`.`dealer_name`), '') as 'dealer', `parts`.`measure` as 'measure', `parts`.`unit_price` as 'unit_price', `parts`.`quantity` as 'quantity', `parts`.`freight` as 'freight', `parts`.`amount` as 'amount', `parts`.`tax` as 'tax', `parts`.`total_amount` as 'total_amount', `parts`.`discount_price` as 'discount_price', `parts`.`net_part_price` as 'net_part_price' FROM `parts` LEFT JOIN `parts_type` as parts_type1 ON `parts_type1`.`part_type_id`=`parts`.`part_type` LEFT JOIN `manufacturer` as manufacturer1 ON `manufacturer1`.`manufacturer_id`=`parts`.`manufacturer` LEFT JOIN `dealer` as dealer1 ON `dealer1`.`dealer_id`=`parts`.`dealer`  WHERE `parts`.`parts_id`='{$id}' limit 1", $eo);
			$row = db_fetch_assoc($res);
			?>
			$j('#part_name_3<?php echo $rnd1; ?>').html('<?php echo addslashes(str_replace(["\r", "\n"], '', safe_html($row['part_name']))); ?>&nbsp;');
			$j('#part_name_4<?php echo $rnd1; ?>').html('<?php echo addslashes(str_replace(["\r", "\n"], '', safe_html($row['part_name']))); ?>&nbsp;');
			$j('#part_name_5<?php echo $rnd1; ?>').html('<?php echo addslashes(str_replace(["\r", "\n"], '', safe_html($row['part_name']))); ?>&nbsp;');
			<?php
			break;
